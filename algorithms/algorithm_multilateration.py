...
class Multilateration():
    """
    Estimates the location of a beacon using multilateration
    """
    def __init__(self, db_client, test_accuracy):
    ...
    def algorithm(self):
        ...
        if self.test_accuracy:
            ...
            for i, timestamp in enumerate(timestamps_test_set):
                ...
                predicted_locations.append(self.predict(data))
        ...
        # For each device_id:
        predicted_locations.append(self.predict(data))
        # self.db_client.testdb.calibrationEstimatedPosition.insert_many(predictedLocations)
    ...
    def predict(self, data):
        ...
        if len(data) == 1:
            ...
            data['dist'] = self.rssi_to_dist(data['rssi'])
            data = data.sort_values(by=['dist']).iloc[0:2, :]
            weight1 = data['dist'].iloc[0] / (data['dist'].iloc[0] +
                                              data['dist'].iloc[1])
            weight2 = data['dist'].iloc[1] / (data['dist'].iloc[0] +
                                              data['dist'].iloc[1])
            lat = data['gatewayLat'].iloc[0] * weight2 + data[
                'gatewayLat'].iloc[1] * weight1
            lng = data['gatewayLng'].iloc[0] * weight2 + data[
                'gatewayLng'].iloc[1] * weight1
    ...
    def multilateration(self, distances_to_gateways, gateways):
        """
        ...
        for gateway in gateways:
            x_coord, y_coord, col, row = utm.from_latlon(
                gateway[0], gateway[1])
            stations.append([x_coord, y_coord])

        stations = list(np.array(stations))
        solution = self.gps_solve(distances_to_gateways, stations)
    ...
    def rssi_to_dist(self, rssi):
        ...
        environ = 1.14  # Environmental factor
        return 10**((measured_power - rssi) / (10 * environ))
